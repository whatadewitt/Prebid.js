...
function send() {
  ...
  */
  ...
  ajax(NewsIQUrl, payloadNewsIQ, (result) =>
    logMessage("Sent NewsCorp Prebid Analytics:", result)
  );
...
window.ProgrammaticBidding.device = getDevice();
...
function getBidParms(bid) {
  return [{ key: "ad unit", value: getAdUnit(bid.adUnitCode) }];
}
